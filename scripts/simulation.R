...
simul<-function(model, data, seed=123, nreps=1000){
  require(mvtnorm)
  set.seed(seed)
...
conf<-function(case1, case2, confidence=.975){
  ...
  return(cbind("effect"=effect, "effect.lb"=effect.lb, "effect.ub"=effect.ub))
...
malepred.list<-t(sapply(male.list, function(x) simul(model, x)))

femalepred.list<-t(sapply(female.list, function(x) simul(model, x)))
